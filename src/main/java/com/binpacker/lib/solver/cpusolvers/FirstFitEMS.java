...
public class FirstFitEMS implements SolverInterface {
		...
		for (Box box : boxes) {
			...
			for (Bin bin : activeBins) {
				...
				for (int i = 0; i < bin.freeSpaces.size(); i++) {
					...
					if (fittedBox != null) {
						Box placedBox = PlacementUtils.placeBoxEMS(fittedBox, bin, i);
						PlacementUtils.pruneCollidingSpacesEMS(placedBox, bin);
						placed = true;
						...
						break; // Break from the spaces loop
					}
				}

				if (placed) {
					break; // Break from the activeBins loop
				}
			}
